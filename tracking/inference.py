...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #For all possible ghost positions, obtain the manhattan distance and include in allPossible counter object
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            if emissionModel[trueDistance] > 0:         
                allPossible[p] = emissionModel[trueDistance] * self.beliefs[p]

        #Handling jail edge case where noisyDistance is None
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        #loops over a list of possible ghost positions   
        for legalPos in self.legalPositions:
            
            #obtains the distribution over new positions for the ghost, given its previous position (legalPos) as well as Pacman's current position
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, legalPos))            
            oldBelief = self.beliefs[legalPos]

            #loops over the new positions
            for newPos in newPosDist.keys():
                #updates beliefs for a time step elapse, which were initially taken with a uniform distribution over ghost positions."
